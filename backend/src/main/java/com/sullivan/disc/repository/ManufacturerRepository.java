package com.sullivan.disc.repository;

/**
 * <h1>ManufacturerRepository</h1>
 * @author Shawn Sullivan
 * CEN 3024C-31774
 * July 8, 2025
 * The ManufacturerRepository allows the upper layers of the Spring REST API to interact with the Manufacturers table
 * of the DB directly and provides several autogenerated SQL query methods
 * Note: Spring-managed repositories are temporarily unused due to project requirement for dynamic user DB config.
 * Kept in code for future use because I would like to expand on this project and/or use it as a learning tool
 * later.
 */

import com.sullivan.disc.model.Manufacturer;
import org.springframework.data.jpa.repository.JpaRepository;

import java.util.Optional;

public interface ManufacturerRepository extends JpaRepository<Manufacturer, Integer> {

//    Optional<Manufacturer> findByManufacturer(String manufacturerName);
}
