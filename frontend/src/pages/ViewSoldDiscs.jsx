...
function ViewReturnedDiscs() {
    ...
    useEffect(() => {
        fetch('http://localhost:8080/api/discs/sold')
            .then(response => response.json())
            .then(data => setDiscs(data))
            .catch(error => console.error('Error fetching sold discs:', error));
    }, []);
    const totalResale = discs.reduce((sum, disc) => sum + disc.resaleValue, 0).toFixed(2);
    ...
    return (
        <div className="table-container">
            ...
            <p><strong>Total Revenue from Sold Discs:</strong> ${totalResale}</p>
